#!/usr/bin/env python3
"""F2_sidebyside_register_diagram.py

Generate an SVG diagram with two side-by-side panels, each showing the
scaffold DNA sequence ruler and scaffold protein footprint.  The LEFT
panel displays forward sliding windows; the RIGHT panel displays reverse
sliding windows.  Windows are colored by min_dist using the cividis
colormap.

Outputs:
  {guest_code}_sidebyside_register_diagram.svg

Usage:
    python F2_sidebyside_register_diagram.py <guest_code> [scaffold_code]
    python F2_sidebyside_register_diagram.py 1jgg 9YZJ
"""
...
svg_path = f'{outdir}/{guest_code}_sidebyside_register_diagram.svg'
...
BRACE_FONT = 3.12    # font size for brace labels
SCALE_H    = 3.0     # scale bar height
SCALE_GAP  = 6.0     # gap before scale bar
PANEL_GAP  = 12.0    # horizontal gap between the two panels
...
# Vertical positions (shared by both panels, in panel-local coords)
...
y_top      = y_num + 4.5
y_bot      = y_top + 3.5
y_bars0    = y_bot + 5.0             # first bar top
# Use n_max so both panels' column markers extend to the same depth
...
# ── Helper: draw one complete panel ──────────────────────────────────
...
scale_x = (total_w - scale_w) / 2   # centered
n_steps = 64
